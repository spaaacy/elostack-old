...
const KeyBenefitsSection = () => {
  ...
  return (
    <section id="key-benefits" className="pt-20 pb-32 overflow-hidden text-black bg-white">
      <div className="container mx-auto px-4">
        <div className="text-center mb-20">
          <h2 className="text-5xl font-bold text-blueprimary" data-aos="zoom-in-down">
            Transform Your Hiring Process
          </h2>
          <p className="mt-4 text-xl max-w-xl mx-auto" data-aos="zoom-in-down" data-aos-delay="100">
            EloStack offers a seamless bridge between top talent and leading companies.
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {benefits.map((benefit, index) => (
            <div key={index} className="p-4" data-aos={benefit.aos} data-aos-delay={benefit.delay}>
              <div className="h-full bg-white bg-opacity-10 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-500 ease-in-out transform hover:scale-105">
                <div className="p-6">
                  <div className="flex items-center justify-center text-center mb-4">
                    {benefit.icon}
                    <h3 className="ml-3 text-2xl font-semibold text-gray-900">{benefit.title}</h3>
                  </div>
